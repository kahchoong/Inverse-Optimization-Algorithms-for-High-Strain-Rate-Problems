...
* <b>What can we learn or takeaway?</b> There are many high-strain rate plasticity models out there that consider different types of variables (temperature, microstructure, hardening laws, etc.) and may only be useful for a given set of state variables. An optimizer can help us gather evidence on if a particular experimental output or model parameters are sufficiently sensitive. Which can, in turn, help us build better models by truncating/adding certain terms or parameters, changing their relationships (logarithmic, arrhenius, linear, etc.), or building a better experiment and collected output that responds more favorably.
* 
